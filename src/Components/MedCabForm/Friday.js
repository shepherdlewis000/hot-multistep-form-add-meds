...
export default function Friday(props){
    ...
    const addTime = () => {
        ...
        let currentFridayTimes = props.getState('fridayTimes');

        if (!currentFridayTimes) { // If FridayTimes not yet initialized, just add hrToAdd
            props.setState('fridayTimes', [hrToAdd]);
        }
       
        /* If fridayTimes already exists and it doesn't already have that time, then add */
        if ((currentFridayTimes) && (!currentFridayTimes.includes(hrToAdd))) {
            let newFridayTimes = [...currentFridayTimes, hrToAdd].sort((a, b) => a - b);
            props.setState('fridayTimes', newFridayTimes);
        }
        ...
        return (
            <>
                <p>Your schedule will be:
                    ...
                    {props.getState('fridayTimes').map((time, index) => 
            ...
            <Row className="mt-3">
                        ...
                        <Card.Footer>
                            ...
                            <Row>
                                <Col>
                                    ...
                                    <Button variant={"danger"} block onClick={() => props.setState('fridayTimes', [])}>Clear stored values</Button>
                                </Col>
                            </Row>
                ...
                <Col md={8}>
                    <ListGroup className={"text-left"}>
                        ...
                        <ListGroup.Item>
                            When you are finished entering all your times, click "Proceed" at the bottom to proceed to Saturday. Or you may use the weekday buttons to jump directly to specific days.
                        </ListGroup.Item>
    ...
    function validate() {
        const currFridayTimes = props.getState('fridayTimes');
        /*if (!currFridayTimes || currFridayTimes.length === 0) {
        }*/
        (currFridayTimes && currFridayTimes.length > 0) ?
            (function () {
                props.jump(10);
            })()
            :
    ...
    function FridayErrorModal(props) {
        return (
            <Modal
                {...props}
                animation={false}
            ...
            >
                ...
                <Modal.Footer>
                    ...
                    <Button onClick={() => props.myjump(10)}>Continue Anyway</Button>

                </Modal.Footer>
            </Modal>
        );
    } // end Friday2ErrorModal
