...
export default function Tuesday(props){
    ...
    const addTime = () => {
        ...
        let currentTuesdayTimes = props.getState('tuesdayTimes');

        if (!currentTuesdayTimes) { // If TuesdayTimes not yet initialized, just add hrToAdd
            props.setState('tuesdayTimes', [hrToAdd]);
        }
       
        /* If tuesdayTimes already exists and it doesn't already have that time, then add */
        if ((currentTuesdayTimes) && (!currentTuesdayTimes.includes(hrToAdd))) {
            let newTuesdayTimes = [...currentTuesdayTimes, hrToAdd].sort((a, b) => a - b);
            props.setState('tuesdayTimes', newTuesdayTimes);
        }
    ...
    function timesList() {
        if (!props.getState('tuesdayTimes'))
            return null;
        /*
        return (
            <>
                <p>Your schedule will be:
                    ...
                    {props.getState('tuesdayTimes').map((time, index) => 
            ...
            <Row className="mt-3">
                        ...
                        <Card.Footer>
                            ...
                            <Row>
                                <Col>
                                    ...
                                    <Button variant={"danger"} block onClick={() => props.setState('tuesdayTimes', [])}>Clear stored values</Button>
                                </Col>
                            </Row>
                ...
                <Col md={8}>
                    <ListGroup className={"text-left"}>
                        ...
                        <ListGroup.Item>
                            When you are finished entering all your times, click "Proceed" at the bottom to proceed to Tuesday. Or you may use the weekday buttons to jump directly to specific days.
                        </ListGroup.Item>
    ...
    function validate() {
        const currTuesdayTimes = props.getState('tuesdayTimes');
        /*if (!currTuesdayTimes || currTuesdayTimes.length === 0) {
        }*/
        (currTuesdayTimes && currTuesdayTimes.length > 0) ?
            (function () {
                props.jump(7);
            })()
            :
    ...
    function TuesdayErrorModal(props) {
        return (
            <Modal
                {...props}
                animation={false}
            ...
            >
                ...
                <Modal.Footer>
                    ...
                    <Button onClick={() => props.myjump(7)}>Continue Anyway</Button>

                </Modal.Footer>
            </Modal>
        );
    } // end Tuesday2ErrorModal

} // end function Tuesday
