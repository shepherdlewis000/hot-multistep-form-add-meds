...
export default function Thursday(props){
    ...
    const addTime = () => {
        ...
        let currentThursdayTimes = props.getState('thursdayTimes');

        if (!currentThursdayTimes) { // If ThursdayTimes not yet initialized, just add hrToAdd
            props.setState('thursdayTimes', [hrToAdd]);
        }
       
        /* If thursdayTimes already exists and it doesn't already have that time, then add */
        if ((currentThursdayTimes) && (!currentThursdayTimes.includes(hrToAdd))) {
            let newThursdayTimes = [...currentThursdayTimes, hrToAdd].sort((a, b) => a - b);
            props.setState('thursdayTimes', newThursdayTimes);
        }
    ...
    function timesList() {
        if (!props.getState('thursdayTimes'))
            return null;
        /*
        return (
            <>
                <p>Your schedule will be:
                    ...
                    {props.getState('thursdayTimes').map((time, index) => 
            ...
            <Row className="mt-3">
                        ...
                        <Card.Footer>
                            ...
                            <Row>
                                <Col>
                                    ...
                                    <Button variant={"danger"} block onClick={() => props.setState('thursdayTimes', [])}>Clear stored values</Button>
                                </Col>
                            </Row>
                ...
                <Col md={8}>
                    <ListGroup className={"text-left"}>
                        ...
                        <ListGroup.Item>
                            When you are finished entering all your times, click "Proceed" at the bottom to proceed to Friday. Or you may use the weekday buttons to jump directly to specific days.
                        </ListGroup.Item>
    ...
    function validate() {
        const currThursdayTimes = props.getState('thursdayTimes');
        /*if (!currThursdayTimes || currThursdayTimes.length === 0) {
        }*/
        (currThursdayTimes && currThursdayTimes.length > 0) ?
            (function () {
                props.jump(9);
            })()
            :
    ...
    function ThursdayErrorModal(props) {
        return (
            <Modal
                {...props}
                animation={false}
            ...
            >
                ...
                <Modal.Footer>
                    ...
                    <Button onClick={() => props.myjump(9)}>Continue Anyway</Button>

                </Modal.Footer>
            </Modal>
        );
    } // end Thursday2ErrorModal

} // end function Thursday
